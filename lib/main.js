...
function compileRegExps(options) {
  return {
    INDENTATION: /^[\s\t]*/,
    BLOCK_OPENING_EXCEPTION: (function() {
      var option = options.blockOpeningException || '[{\\[\\(,]';
      return new RegExp(option +'[\\s\\t]*$');
    })(),
    PARENS_CLOSING: /^[\s\t]*(\).*)$/
  };
}
...
function blockStart(block, output, line, indent, options, re) {
  ...
  if (block.previousLine != null) {
    // don't open curly
    if (re.BLOCK_OPENING_EXCEPTION.test(block.previousLine)) {
